...
insert_mysql_register(struct user *user_info, struct mysql_config *mysql_config_info)
{
	...
	sprintf(sql_insert, "insert into %s (id, name,password) values('%d', '%s', '%s')",
		mysql_config_info->user_register, id, user_info->user_name, user_info->user_password);
	mysql_init(&m_conn);
	if (mysql_real_connect(&m_conn, "localhost", "root", "fl1328", mysql_config_info->database, 0, NULL,
			       0)) {
		printf("mysql connect success!\n");
...
insert_mysql(struct mysql_config *insert_info, User *user_info)
{
	...
	sprintf(sql_insert,
		"insert into %s (id, name, sex, age, password, address, time) values('%d', '%s', '%s', '%d', '%s', '%s', '%s')",
		insert_info->info_table, user_info->user_id, user_info->user_name, user_info->user_sex,
		user_info->user_age, user_info->user_password, user_info->user_address, user_info->user_date);
	mysql_init(&m_conn);
...
void
select_mysql_sign_in(struct mysql_config *sign_in, const char *user_name,
		     const char *user_password)
{
	...
	sprintf(sql_select, "select name from %s where password = %s", sign_in->user_register,
		user_password);
	mysql_init(&m_conn);
...
alter_table(struct mysql_config *alter, char *queue_name, char *queue_type)
{
	...
	sprintf(alter_table_queue, "alter table %s add column %s %s", alter->info_table, queue_name,
		queue_type);
	mysql_init(&m_conn);
